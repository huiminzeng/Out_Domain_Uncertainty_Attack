...
parser.add_argument('--mode', default='deep_ensemble_at', type=str)
parser.add_argument('--seed', default=0, type=int)

parser.add_argument('--epsilon', default=0.1, type=float)

def adv_loss(model, x_natural, y,
             optimizer, args):
    ...
    outputs, predictions = model(x_natural)
    outputs_adv, _ = model(x_adv)
    # Take mean directly, regardless reduction
    loss_natural = criterion_ce(outputs, y)
    loss_adv = criterion_ce(outputs_adv, y)
...
def main():
    ...
    save_dir = os.path.join(args.save_path, args.mode, args.model, 'eps_' + str(args.epsilon), 'seed_' + str(args.seed))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        loss, predictions = adv_loss(model, inputs, targets, optimizer, args)
